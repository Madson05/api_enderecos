...
export const refactorResult = (result: any, data?: GetPessoaType) => {
  ...
  if (result && result.length > 0) {
    if (data && (data.codigoPessoa !== undefined)) {
      ...
      const pessoa = new PessoaEntity(Number(dataPessoa[0]), String(dataPessoa[1]), String(dataPessoa[2]), Number(dataPessoa[3]), String(dataPessoa[4]), String(dataPessoa[5]), String(dataPessoa[6]));
