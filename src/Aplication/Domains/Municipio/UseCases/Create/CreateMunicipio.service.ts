...
class CreateMunicipioService {
  ...
  async execute(data: CreateMunicipioType): Promise<any> {
    const checkExists = await this.municipioRepository.checkExistsByNome(data.nome, data.codigoUF);
    ...
    const municipio = new MunicipioEntity(await getNextSequence("SEQUENCE_MUNICIPIO"), data.codigoUF, data.nome, data.status);
